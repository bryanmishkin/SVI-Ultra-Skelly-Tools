...
def get_wifi_mac(): return build_cmd("CC") # NOT WORKING (Probably because of missing Wi-Fi support in the device)
...
# ============
def start_send_data(size: int, max_pack: int, filename: str):
    return build_cmd("C0", int_to_hex(size, 4) + int_to_hex(max_pack, 2) + "5C55" + to_utf16le_hex(filename))

def send_data_chunk(index: int, data: bytes):
...
def set_music_animation(action: int, cluster: int, filename: str):
    name_utf16 = to_utf16le_hex(filename)
    name_len = int_to_hex((len(name_utf16)//2)+2, 1) if filename else "00"
    payload = int_to_hex(action,1) + "00" + int_to_hex(cluster,4) + (name_len + "5C55" + name_utf16 if filename else name_len)
    return build_cmd("CA", payload)

# ==========
# Wi-Fi Commands
# ==========
def set_wifi_power(power: int):
    return build_cmd("CB", int_to_hex(power,1))

# =======================
# Connection & Example Use
# =======================
...
def handle_notification(sender, data):
    ...
    if hexstr.startswith("BBCC"):
        mac = hexstr[4:16]
        print(f"[PARSED] Wi-Fi MAC Address: {mac}")
        return

    if hexstr.startswith("BBC0"):
        failed = int(hexstr[4:6], 16)
        written = int(hexstr[6:14], 16)
        print(f"[PARSED] Start Transfer - Failed: {failed}, Written Length: {written}")
        return

    if hexstr.startswith("BBC1"):
        dropped = int(hexstr[4:6], 16)
        index = int(hexstr[6:10], 16)
        print(f"[PARSED] Chunk Dropped - Dropped: {dropped}, Index: {index}")
        return

    if hexstr.startswith("BBC2"):
        failed = int(hexstr[4:6], 16)
        print(f"[PARSED] Transfer End - Failed: {failed}")
        return

    if hexstr.startswith("BBC3"):
        failed = int(hexstr[4:6], 16)
        print(f"[PARSED] Rename Complete - Failed: {failed}")
        return

    if hexstr.startswith("BBC4"):
        failed = int(hexstr[4:6], 16)
        print(f"[PARSED] Cancel Transfer - Failed: {failed}")
        return

    if hexstr.startswith("BBC5"):
        written = int(hexstr[4:12], 16)
        print(f"[PARSED] Resume Write Length: {written}")
        return
...
async def run():
    ...
    async with BleakClient(device) as client:
        ...
        # Example: Query volume
        #wait send_command(client, query_volume())

        #await send_command(client, query_bt_name())
        #await send_command(client, query_device_parameter())
        #await send_command(client, query_live_mode())
        #await send_command(client, enable_classic_bt())
        #await send_command(client, get_wifi_mac())
        #await send_command(client, query_capacity())
        #await send_command(client, set_wifi_power(1))
        #await send_command(client, set_brightness(-1, 255, 0, None))
        #await send_command(client, set_rgb(-1, 255, 0, 0, 0, 0, None))
        #await send_command(client, set_eye_icon(1, 0, None))


        # Not Fully tested
        #await send_command(client, play_or_pause_file(1,1)) 
        #await send_command(client, set_music_animation(255, 0, None))
        #await send_command(client, set_music_mode(1))

        # Have not tried any of the file stuff.
